...
class Config:
    ...
    # Optional log2(opacity) evaluation path.
    # Keep this False unless your compiled CUDA kernels explicitly support it.
    # This avoids needing CUDA code edits/rebuilds just to switch train vs eval.
    eval_use_optimized_raster: bool = False
...
class Runner:
    ...
    def rasterize_splats(
        ...
        early_sorting = False, 
        use_optimized_opacity: bool = False,
        **kwargs,
        ...
        if use_optimized_opacity:
            opacities = torch.log2(opacities.clamp_min(1e-12))
        ...
        use_training_kernels = not use_optimized_opacity
        render_colors, render_alphas, info, t = rasterization(
            means=means,
            quats=quats,
            scales=scales,
            opacities=opacities,
            ...
            early_sorting = early_sorting,
            use_training_kernels=use_training_kernels,
            **kwargs,
        )
        if masks is not None:
            render_colors[~masks] = 0
        return render_colors, render_alphas, info, t

    def _sorting_decision_paths(self, step: int, base_dir: Optional[str] = None) -> List[str]:
        """Candidate file paths for sorting decision at a training iteration."""
        root = self.ckpt_dir if base_dir is None else base_dir
        return [
            f"{root}/sorting_decision_step{step}.json",
            f"{root}/sorting_decision_{step}.json",  # backward compatibility
        ]

    def _save_sorting_decision(self, step: int, time_original: float, time_early: float):
        """Save sorting strategy decision for this checkpoint iteration."""
        use_early_sorting = time_early < time_original
        payload = {
            "step": int(step),
            "scene": os.path.basename(os.path.normpath(self.cfg.data_dir)),
            "use_early_sorting": bool(use_early_sorting),
            "time_original": float(time_original),
            "time_early_sorting": float(time_early),
            "time_delta": float(time_original - time_early),
            "faster_method": "early_sorting" if use_early_sorting else "original_sorting",
        }
        path = self._sorting_decision_paths(step)[0]
        with open(path, "w") as f:
            json.dump(payload, f, indent=2)
        return path, use_early_sorting

    def _load_sorting_decision(self, step: int) -> Tuple[bool, Optional[str], Optional[dict]]:
        """Load sorting strategy decision for this checkpoint iteration."""
        base_dir = self.ckpt_dir
        if self.cfg.ckpt:
            base_dir = os.path.dirname(self.cfg.ckpt[0])

        for candidate in self._sorting_decision_paths(step, base_dir=base_dir):
            if os.path.exists(candidate):
                try:
                    with open(candidate, "r") as f:
                        decision = json.load(f)
                    return bool(decision.get("use_early_sorting", False)), candidate, decision
                except Exception:
                    continue
        return False, None, None

    def _append_jsonl(self, path: str, payload: dict) -> None:
        """Append one JSON object as a new line (JSONL)."""
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            # forward
            renders, alphas, info, _ = self.rasterize_splats(
            ...
            if step in [i - 1 for i in cfg.save_steps] or step == max_steps - 1:
                ...
                # Benchmark both sorting methods at every checkpoint save step.
    ...
    def eval(
        ...
        # Determine sorting policy for this evaluation.
        if force_es is not None:
            es = bool(force_es)
            decision_file = None
            if world_rank == 0:
                print(f"[{stage}] Forcing early_sorting={es}")
        else:
            es, decision_file, _ = self._load_sorting_decision(step)
            if world_rank == 0:
                if decision_file is not None:
                    print(f"[{stage}] Loaded sorting decision: early_sorting={es} from {decision_file}")
                else:
                    print(f"[{stage}] No sorting decision file for step {step}; using early_sorting=False")
        ...
        for run_idx in range(runs):
            ...
            for i, data in enumerate(valloader):
                ...
                colors, _, _, t = self.rasterize_splats(
                    camtoworlds=camtoworlds,
                    Ks=Ks,
                    width=width,
                    height=height,
                    ...
                    masks=masks,
                    early_sorting=es,
                    use_optimized_opacity=cfg.eval_use_optimized_raster,
                )  # [1, H, W, 3]
                pts.append(float(t["pt"]))
                its.append(float(t["it"]))
                ...
                if world_rank == 0 and not skip_render and run_idx == 0:
                    ...
                    imageio.imwrite(
                        f"{self.render_dir}/{stage}_{es_str}_step{step}_{i:04d}.png",
                        canvas,
                    )
