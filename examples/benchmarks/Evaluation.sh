...
# Auto-detect CKPT_ROOT if not explicitly set via environment variable.
if [[ -z "${CKPT_ROOT:-}" ]]; then
    if [[ -d "results/New_Training" ]]; then
        CKPT_ROOT="results/New_Training"
        echo "[Auto] Found results/New_Training — using it as CKPT_ROOT."
    elif [[ -d "results/Benchmark_Training" ]]; then
        CKPT_ROOT="results/Benchmark_Training"
        echo "[Auto] Found results/Benchmark_Training — using it as CKPT_ROOT."
    else
        echo "[ERROR] No checkpoint folder found in results/."
        echo "        You can either:"
        echo "          1) Train your own checkpoints by running the Training.sh file."
        echo "          2) Download the pre-trained checkpoints from:"
        echo "             https://zenodo.org/records/18943926/files/Benchmark_Training.tar.gz?download=1"
        exit 1
    fi
fi
