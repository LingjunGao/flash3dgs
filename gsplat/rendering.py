...
# Import Profiling to record the gpu utilization
...
def rasterization(
    ...
    tic = time.time()
    if early_sorting:
        depths = depths.argsort(dim=1, stable=True).to(torch.int32)  # the one cast
    ...
    isect_offsets = isect_offset_encode(isect_ids, C, tile_width, tile_height, early_sorting)
    meta.update(
        {
    ...
    torch.cuda.synchronize()
    #view_dir = extract_view(viewmats, N)                # C, N, 3
    #mlp_input = torch.cat([depths.unsqueeze(-1), view_dir, opacities.unsqueeze(-1)], dim = -1) # C, N, 5 
    #depth_parameters = mlp(mlp_input)
    sort_time = time.time() -tic
    tic = time.time()
    ...
    t = {"pt":proj_time,
         ...
         "rt": raster_time,
         "tt": total_time}
    return render_colors, render_alphas, meta, t
...
def _rasterization(
    ...
    # print(f"sum_tiles_per_gs{tiles_per_gauss.sum().item()}")
    # print(f"gs_count={tiles_per_gauss.shape}")
    isect_offsets = isect_offset_encode(isect_ids, C, tile_width, tile_height)
    # depth_id = isect_ids & 0xFFFFFFFF
    # depth_float = torch.zeros_like(isect_ids, dtype=torch.float32)
    # for i in range(len(depth_id)):
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        render_colors = _rasterize_to_pixels(
            self,
            viewmats,
            means2d,
            conics,
...
def extract_view(viewmatrix: torch.Tensor,
                 N: int,) -> torch.Tensor:
        
        cam2world = torch.inverse(viewmatrix)  # (C, 4, 4)

        view_dir = -cam2world[:, :3, 2].unsqueeze(1)  # (C, 1, 3)
        view_dir = view_dir / torch.norm(view_dir, dim=-1, keepdim=True)
        view_dir = view_dir.expand(viewmatrix.shape[0], N, -1)
