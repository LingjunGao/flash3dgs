...
def isect_offset_encode(
    ...
    return _make_lazy_cuda_func("intersect_offset")(
        isect_ids.contiguous(), n_cameras, tile_width, tile_height, early_sorting
    )
...
class _FullyFusedProjection(torch.autograd.Function):
    ...
    def forward(
        ...
        op_name = (
            "projection_ewa_3dgs_fused_fwd_train"
            if use_training_kernels
            else "projection_ewa_3dgs_fused_fwd"
        )
        ...
        ctx.eps2d = eps2d
        ctx.camera_model_type = camera_model_type
        ctx.use_training_kernels = use_training_kernels
    ...
    def backward(ctx, v_radii, v_means2d, v_depths, v_conics, v_compensations):
        ...
        op_name = (
            "projection_ewa_3dgs_fused_bwd_train"
            if ctx.use_training_kernels
            else "projection_ewa_3dgs_fused_bwd"
        )
...
class _RasterizeToPixels(torch.autograd.Function):
    ...
    def forward(
        ...
        flatten_ids: Tensor,  # [n_isects]
        absgrad: bool,
        use_training_kernels: bool,
    ) -> Tuple[Tensor, Tensor]:
        colors16 = colors# .to(torch.float16)
        op_name = (
            "rasterize_to_pixels_3dgs_fwd_train"
            if use_training_kernels
            else "rasterize_to_pixels_3dgs_fwd"
        )
...
import os
def data_storage(data, name):
    relative_dir = "../rasterization_bench/data_storage//back/"
    name += ".pt"
    file_path = os.path.join(relative_dir, name)
    os.makedirs(relative_dir, exist_ok=True)
    if not os.path.exists(file_path):
        torch.save(data, file_path)
    else:
        print(f"There are somehting in the file {relative_dir} \n")