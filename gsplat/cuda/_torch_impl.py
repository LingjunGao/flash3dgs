...
def accumulate(
    ...
    # ) *  torch.exp(-a*torch.pow(depths[camera_ids, gaussian_ids], b))

    # alphas = torch.clamp_max(
    #     opacities[camera_ids, gaussian_ids] * torch.exp(-sigmas), 0.999
    # ) *  torch.relu(1 - depths[camera_ids, gaussian_ids] / a) * b

    batch_opacities = opacities[camera_ids, gaussian_ids].reshape(-1, 1)
    batch_depths = depths[camera_ids, gaussian_ids].reshape(-1, 1)
    batch_sigmas = sigmas.reshape(-1, 1)

##original


    view_dir, cam_pos = extract_view(viewmats,batch_depths.shape[0],True)


    # print(f"batch_depths shape={batch_depths.shape}")

    # 全量计算 alphas
    # alphas = torch.clamp_max(batch_opacities * torch.exp(-batch_sigmas), 0.999) * mlp(batch_depths, view_dir, batch_opacities)
### out of memory
# 可调节的每次处理样本数
    L = batch_depths.shape[0]
    alphas = batch_opacities.new_empty(L, 1)  # 预分配
    chunk_size = 1000_000
    ...
    for i in range(chunks):
        ...
        d_chunk = batch_depths[start:end]
        op_chunk = batch_opacities[start:end]
        sigma_chunk = batch_sigmas[start:end]
        dir_chunk = view_dir[start:end]

        alpha_chunk = torch.clamp_max(op_chunk * torch.exp(-sigma_chunk), 0.999) \
                    * checkpoint(mlp, d_chunk, dir_chunk, op_chunk, use_reentrant=False)

        alphas[start:end] = alpha_chunk  # 直接写回
        
        torch.cuda.empty_cache()

    # 拼接所有结果
    # alphas = torch.cat(alpha_list, dim=0)  # (..., 1)
####
    ...
    # 检查最终结果
    # print("Final alphas shape:", alphas.shape)
    ...
    # )
    alphas_per_pixel = accumulate_along_rays(
        alphas,
        None,  # 仅累加alphas本身
        ray_indices=indices,
        n_rays=total_pixels,
    ...
    renders = accumulate_along_rays(
        ...
        ray_indices=indices,
        n_rays=total_pixels,
    ).reshape(C, image_height, image_width, channels)/(alphas_per_pixel.reshape(C, image_height, image_width, 1) + 1e-10)
...
def _rasterize_to_pixels(
    ...
    min_range = (isect_offsets_fl[1:] - isect_offsets_fl[:-1]).min().item()
    mean_range = (isect_offsets_fl[1:] - isect_offsets_fl[:-1]).float().mean().item()
    sum =  (isect_offsets_fl[1:] - isect_offsets_fl[:-1]).float().sum().item()
    # print(f"max_range={max_range}")
    # print(f"min_range={min_range}")
    # print(f"mean_range={mean_range}")
    # print(f"sum={sum}")
    self.max_list.append(max_range)
    self.min_list.append(min_range)
    self.mean_list.append(mean_range)
    ...
    for step in range(0, num_batches, batch_per_iter):
        ...
        # print(f"gs_ids.shape={gs_ids.shape}")
        if len(gs_ids) == 0:
            break
...
def extract_view(viewmatrix: torch.Tensor, batch_size: int, include_pos: bool=True) -> torch.Tensor:
        
        cam2world = torch.inverse(viewmatrix[0])  # (4, 4)

        view_dir = -cam2world[:3, 2].unsqueeze(0)  # (1, 3)
        view_dir = view_dir / torch.norm(view_dir, dim=-1, keepdim=True)
        view_dir = view_dir.expand(batch_size, -1)

        if include_pos:
            cam_pos = cam2world[:3, 3].unsqueeze(0)  # (1, 3)
            cam_pos = cam_pos.expand(batch_size, -1)
            return view_dir, cam_pos
        else:
